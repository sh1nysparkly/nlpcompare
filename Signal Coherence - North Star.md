...
### Immediate-sprint Lab capability gaps (May 17 recalibration)

**This section supersedes the previous framing that listed Slot × intent as the highest-leverage Phase 2 pull-forward.** Recalibrated May 17 after Anna re-grounded on what would actually make the next round of Track-2 work less painful: the Lab's existing capability set has gaps that bite harder *for the immediate sprint* than the Phase 2 conceptual pull-forwards below. Fix the gaps; Track 2 work flows.

1. **V0 overwrite.** Currently stubbed -- `getCommitOverwriteTarget()` recognizes V0 but `showSavePermInput` falls through with a toast. The fix is schema-shaped: add `is_live_baseline` (or similar) column to `permutations`, then add `commitOverwriteV0()` paralleling `commitOverwriteV05()`. **Load-bearing for being able to update/evolve the live baseline as new GSC-faithful curation lands.** Without it, V0 is frozen and every correction means save-as-new + manual version-juggling.
2. **Captured-ideas backlog** (canonical list in `memory/lab-state-and-followups.md`, "Other fixes to capture" section). 16+ workflow improvements Anna has been jotting across sessions -- favicon, Lab rename, slide-in sidebar ToC, entity count per version + per container, auto-inherit container heading from top text, version-win plain-language export, entity colour-coding by type, click-entity-highlight-containers, permutation-aware edit state, card duplication, block deletion, in-line comments, etc. Each is small individually; collectively they reshape the editor from "rearrange + score" into "rearrange + reason + export." Promote case-by-case as Anna picks them up.
3. **Light refactor work that pays cumulatively.** Event delegation (48 inline `onclick=` handlers → one delegated listener on `#block-list`) + stale `labRearrangedNlp` clear in `loadVersionIntoLab`. Time-box together -- ~1 session. NOT the full state-centralization / render-split / drag-drop-module refactor (that's long-game eng investment, see "Architectural threads" in `lab-state-and-followups.md`, only worth it if 6+ more months of building on this codebase are planned).

Execution staging captured in `memory/lab-state-and-followups.md` under "May 17 execution staging."

### Phase 2 bits worth pulling forward for the current sprint

(Reordered May 17 -- the immediate-sprint Lab capability gaps above outrank these for the current sprint window. These remain valid as future work, especially anything that zooms out wider than the page-specific lens.)

1. **Pre-launch HTML loaded into the matrix-of-versions view.** Load the 10 LPs' Aug 2025 HTML into `crawled_pages` (files exist on disk; loader script `signal-coherence/scripts/load_may8_crawl.py` is straightforward to adapt with `source='pre-launch-aug2025'` and same template_type tagging). The matrix view becomes: pre-launch | current (post-launch) | UX proposal | optimized variant — for each page. Stakeholder artifact. Frames the case as "here's the trajectory" rather than "here's what's wrong with UX's proposal." Note: pre-launch is *evidence*, not benchmark (see §9).
2. **Slot × intent diagnostic.** Run the NLP API on each *slot's* content separately (not just whole-page). Output per slot: dominant category + confidence. Surfaces things like *"this commercial LP's hero slot is reading 70% informational"* or *"the Travel Expertise slot on /vacation-packages is leaking 40% Insurance -- your entity-model skew is showing up at the position most likely to influence intent."* Bridge already does the necessary NLP calls; this is per-slot dispatch + UI surfacing. **Useful when zooming out wider than the page-specific lens** (cross-page pattern surfacing) -- less directly load-bearing for the immediate Track-2-brief workflow than the capability gaps above. Exec-legible without statistical literacy. Directly actionable (the fix is in the slot).
