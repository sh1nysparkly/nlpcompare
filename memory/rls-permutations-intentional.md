...
- **`crawled_pages`, `nlp_cache`, `recipe_slots` have RLS-enabled-no-policies** -- correct for those three; they're service-role-only via the `bridge` edge function, so absence of policies blocks anon access (intended). The advisor INFO-level note is noise for them.
- ~~`lp_blocks` also has RLS-enabled-no-policies~~ -- **CORRECTION (May 22):** `lp_blocks` had this posture too, but it was *wrong*, not intentional. The Lab's `loadBlocksFromCuration` and `onContainerPatternChange` call `/rest/v1/lp_blocks` directly with anon, NOT via bridge. The default-deny posture was silently making the Lab fall back to its HTML parser for every page (the parser-fallback masked the bug on rich-classed HTML, surfaced only when Anna landed class-stripped HTML in `/vacation-packages/tours`). Fix applied: `anon_all_access` policy mirroring the one on `permutations`. See `memory/rls-lp-blocks-intentional.md` for the full writeup.
