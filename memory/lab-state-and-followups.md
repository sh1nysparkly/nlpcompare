...
- **Legacy V0.5 permutations lack `block_id`** in their saved blocks (snapshot helper stripped it pre-May 17). Plain-language export diff against them will mark everything NEW/REMOVED. Anna re-saving V0.5 upgrades the row to the new self-contained shape. Could write a one-time backfill migration if it becomes annoying.
...
- **RLS still disabled on `public.permutations`** -- pre-existing security issue surfaced during the May 17 work. Anyone with the anon key can read/modify every row. Not blocking; flag before doing other schema work in that area.
